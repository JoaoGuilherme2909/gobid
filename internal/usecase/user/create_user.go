...
func (req CreateUserReq) Valid(ctx context.Context) validator.Evaluator {
	...
	eval.CheckField(
		validator.MinChars(req.Password, 8), "password", "The password must have at least 8 characters")

	eval.CheckField(validator.NotBlank(req.Bio), "bio", "this field cannot be empty")
	eval.CheckField(
		validator.MinChars(req.Bio, 10) &&
			validator.MaxChars(req.Bio, 255), "bio", "this field must have a lenght between 10 and 255")
